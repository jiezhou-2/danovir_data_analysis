...
ff=data15$feature[1:45]
cc=matrix(nrow=length(ff),ncol = length(ff))
rownames(cc)=ff
colnames(cc)=ff
nn=length(unique(data15$subject_accession))
while(dd<10^(-4)){
mainEffect2=gls(y~arm+ antigen+detect_reagent+ arm:antigen+arm:detect_reagent+antigen:detect_reagent+arm:antigen:detect_reagent,data = data15,method = "ML")
data15$resi=data15$value_reported-mainEffect2$fitted
for (i in ff) {
  for (j in ff) {
    index1=which(data15$feature==i)
    index2=which(data15$feature==j)
    aa1=data15$resi[index1]
    aa2=data15$resi[index2]
    cc[i,j]=cor(aa1,aa2)
  }
}
data15$y=solve(kronecker(diag(nn),cc)) %*% data15$value_reported
