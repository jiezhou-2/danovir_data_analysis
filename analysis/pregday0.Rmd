...
### Mode 1 : full model + independent and heterscedastic error 
...
data15$res=mainEffect2$residuals
feature=unique(data15$feature)
ww=c()
for (i in 1:length(feature)) {
  index=which(data15$feature==feature[i])
  ww=c(ww,1/var(data15$res[index]))
}
wei=cbind(feature,ww)
data15=merge(data15,wei)
...
mm=model.matrix(value_reported~arm+ antigen+detect_reagent+assay+ arm:antigen+arm:detect_reagent+antigen:detect_reagent+arm:antigen:detect_reagent,data = data15)
value_reported=data15$value_reported
weights=data15$ww
cv_results=cv.glmnet(x=mm,y=value_reported,weights = weights)
beta=coef(cv_results$glmnet.fit)[,cv_results$index[1]]
aa=summary(mainEffect2)
```
...
### Model 2: full model + random intercept + heteroscedastic error
...
```{r,message=FALSE,echo=FALSE}
mainEffect3=lme(value_reported~arm+ antigen+detect_reagent+assay+ arm:antigen+arm:detect_reagent+antigen:detect_reagent+arm:antigen:detect_reagent, random=pdDiag(~1),
               weights = varIdent(form = ~1|feature),data = data15,method = "ML",control = contr)
...
```{r,echo=FALSE}
base_data3 |>
  forestplot(labeltext = c(interaction),txt_gp=fpTxtGp(cex=0.2,label = gpar(fontfamily = "HersheySerif")),
             clip = c(-6, 6)) |>
  fp_set_style(box = "royalblue",
               line = "darkblue",
               summary = "royalblue") |> 
...
```{r,message=FALSE,echo=FALSE}
mainEffect3=lme(value_reported~arm+ antigen+detect_reagent+assay+ arm:antigen+arm:detect_reagent+antigen:detect_reagent+arm:antigen:detect_reagent,random=pdDiag(~antigen+detect_reagent),weights = varIdent(form = ~1+antigen),data = data15,method = "ML",control = contr)
aa=summary(mainEffect3)
```
...
### Model 4: full model + random intercept/antibody + homoscedastic error
...
```{r,message=FALSE,echo=FALSE}
mainEffect3=lme(value_reported~arm+ antigen+detect_reagent+assay+ arm:antigen+arm:detect_reagent+antigen:detect_reagent+arm:antigen:detect_reagent,random=pdDiag(~antigen+detect_reagent),weights = varIdent(form = ~1+detect_reagent),data = data15,method = "ML",control = contr)
aa=summary(mainEffect3)
```
